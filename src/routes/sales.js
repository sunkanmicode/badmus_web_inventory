...
router.post('/get/barcodeNumber', guard.ensureLoggedIn(), async (req, res, next) => {
  const pro = await Product.findOne({ barcodeNumber: req.body.barcodeNumber, })
  console.log(pro);
  if (pro !== null) {
    return res.json(pro)
  } else {
...
router.post('/create/customer', guard.ensureLoggedIn(), async (req, res, next) => {
  ...
  if (errors) {
    ...
    res.redirect("/sales/create/sales/pos");
  } else {
...
router.get('/reverse/:saleId', async (req, res) => {
  ...
  Sales.findById(saleId, function (err, sale) { 
    ...
    for (let i = 0; i < sale._productId.length; i++) { 
      BranchProduct.findOne({ _productId: sale._productId[i]}, function (err, increaseProduct) { 
...
router.post('/create/sale', guard.ensureLoggedIn(), async (req, res, next) => {
  ...
  var errors = req.validationErrors();
  console.log(errors, "am the error");
...
console.log(req.user);
      ...
      const deductProduct = await BranchProduct.findOne({
        _productId: req.body.salesArray[i]._productId,
      });

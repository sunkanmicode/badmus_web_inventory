...
router.get('/dashboard', guard.ensureLoggedIn(), async (req, res) => {
  ...
  res.render('admin/dashboard', { user, branch, product, products, account, suppliers, expressFlash: req.flash('success'),
                                  layout: 'layouts/user' });
});
...
router.post('/new-member', guard.ensureLoggedIn(), async (req, res, next) => {
  ...
  form.parse(req, async (err, fields, files) => {
    ...
    if (user) {
      ...
      member.enterProduct =
        fields.enterProduct !== "" ? fields.enterProduct : "";
      if (passport && passport.name) {
        const name = `${Math.round(Math.random() * 10000)}.${passport.name
          .split(".")
          .pop()}`;
      ...
      Account.register(new Account(member), password, async (err, account) => {
        // return false;
        const tokenG = await Account.findOne({
          _id: account._id,
          _storeId: account._storeId,
        });
        //  console.log(tokenG);
        ...
        if (err) {
          ...
          req.flash(
            "success",
            `Saved Successfully! Your Username is ${member.username}`
          );

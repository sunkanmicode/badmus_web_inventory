...
router.get('/view/:productId', guard.ensureLoggedIn(), async (req, res, next) => {
  ...
  const branchproducts = await BranchProduct.find({ _storeId: req.user._storeId, _productId: branchproduct._productId }).populate('_productId').populate('_branchId');
...
router.post('/delete', guard.ensureLoggedIn(), async (req, res) => {
  ...
  const branchProduct = await BranchProduct.findOne({ _productId: id });
  if(branchProduct){
    await branchProduct.delete()
  }

...
router.get("/dashboard", async (req, res) => {
  ...
  if (user.roleId === "admin") {
    ...
    user._roleId.name === "admin" &&
    user._roleId.roleType === "Store"
  ) {
    res.redirect("/admin/dashboard");
  } else if (
    user._roleId.name === "staff" &&
    user._roleId.roleType === "Store"
  ) {
    res.redirect(`/admin/dashboard/`);
  } else if (
    user._roleId.name === "admin" &&
    user._roleId.roleType === "Branch"
  ) {
    res.redirect(
      `/branch/admin/dashboard/${user._storeId._id}/${user._branchId}`
    );
  } else if (
    user._roleId.name === "staff" &&
    user._roleId.roleType === "Branch"
...
router.post(
  ...
  //             Math.abs((expiredDate - presentDate) / (1000 * 60 * 60 * 24))
  //           );
  ...
  //                 ) {
  //                   res.redirect(
  //                     `/staff/dashboard/${user._storeId._id}/${user._branchId}`
  //                   );
  //                 } else if (
  //                   user._roleId.name === "admin" &&
  //                   user._roleId.roleType === "Branch"
  ...
  //           if (user.roleId === "admin") {
  //             res.render("subscription/license", {
  ...
  //     );
  //     if (sub === null) {
  //       const getExpire = await Subscription.findOne({
  //         _storeId: req.user._storeId,
  //       });
...
router.get("/forgot/password", async (req, res) => {
  res.render("store/forgotPassword", {
...
router.post("/forgot/password", async (req, res, next) => {
  ...
  const sendForgotPasswordEmail = async (user) => {
    ...
    let transporter = nodemailer.createTransport({
      ...
      host: "smtp.gmail.com",
      auth: {
        user: "badmus@gmail.com", // generated ethereal user
        pass: "eoFseg@$$3AT", // generated ethereal password
      },
    });
    const mailOptions = {
      to: user.email,
      from: "badmus@gmail.com",
      subject: "Reset your password on Badmus",
...
router.get("/password/reset/:token", async (req, res, next) => {
  const user = await Account.findOne(
    ...
    (err, user) => {
      ...
      res.render("store/passwordReset", {
...
router.post("/password/reset/:token", async (req, res, next) => {
  ...
  if (errors) {
    req.flash(
      "error",
      "Password do not match Or must be at least 4 characters long."
    );
  ...
  await Account.findOne({
    ...
    if (sanitizedUser) {
      sanitizedUser.setPassword(req.body.password, () => {
        ...
        return sanitizedUser.save().then(
          () =>
    ...
    const mailOptions = {
      ...
      from: "badmus@gmail.com",
      subject: "Your Badmus password has been changed",
